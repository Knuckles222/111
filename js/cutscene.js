...
const CUTSCENE = (() => {
  ...
  // ── 타이핑 효과 상태 ─────────────────────────────────
  // typewriterIndex : 현재까지 표시할 글자 수 (소수점 누적용)
  // typewriterSpeed : 프레임당 표시할 글자 수 (클수록 빠름)
  let typewriterIndex = 0;
  let typewriterSpeed = 0.5;  // 기본 속도: 프레임당 1.5글자
  let typewriterDone  = false; // 현재 슬라이드 타이핑 완료 여부
  ...
  function draw() {
    ...
    if (typeof comu !== 'undefined' && comu) {
      image(comu, 0, 200, VIRTUAL_WIDTH, VIRTUAL_HEIGHT - 150);
    }
    ...
    if (!typewriterDone) {
      ...
      if (typewriterIndex >= fullText.length) {
        ...
        typewriterDone  = true;
        // 타이핑 완료 → 사운드 정지
        if (typeof textScrollSound !== 'undefined' && textScrollSound && textScrollSound.isPlaying()) {
          textScrollSound.stop();
        }
      } else {
        // 타이핑 중 → 루프 재생 (첫 슬라이드는 제외)
        if (currentSlide > 0 && typeof textScrollSound !== 'undefined' && textScrollSound && !textScrollSound.isPlaying()) {
          textScrollSound.setVolume(3.0);
          textScrollSound.loop();
        }
      }
    }
    const displayText = fullText.substring(0, Math.floor(typewriterIndex));

    // 대사
    const fullWidth = textWidth(fullText);
    const startX = VIRTUAL_WIDTH / 2 - fullWidth / 2;
    text(displayText, startX, 795);

    // 화자 이름 (효과음 state 는 표시 안 함)
    if (!EFFECT_STATES.includes(st)) {
      textAlign(CENTER, CENTER);  // 화자 이름은 중앙 유지
      text(st, VIRTUAL_WIDTH / 2, 733);
    }

    // ── SPACE >> 안내 (타이핑 완료됐을 때만 표시) ────────
    if (typewriterDone) {
      push();
      textFont(gameFont);
      fill(150);
  ...
  function _advance() {
    ...
    // 타이핑 중이면 → 즉시 전체 표시 (다음 슬라이드로 넘어가지 않음)
    if (!typewriterDone) {
      const slide = data[currentSlide];
      if (slide) {
        typewriterIndex = slide.text.length;
        typewriterDone  = true;
        if (typeof textScrollSound !== 'undefined' && textScrollSound && textScrollSound.isPlaying()) {
          textScrollSound.stop();
        }
  ...
  // ──────────────────────────────────────────────────────
  //  타이핑 효과 초기화 (슬라이드 전환 시 호출)
  // ──────────────────────────────────────────────────────
  function _resetTypewriter() {
    const data  = SCENES[currentSceneId];
    const slide = data ? data[currentSlide] : null;

    // 효과음 슬라이드는 타이핑 없이 즉시 완료 처리
    if (!slide || EFFECT_STATES.includes(slide.state)) {
      typewriterIndex = slide ? slide.text.length : 0;
      typewriterDone  = true;
    } else {
      typewriterIndex = 0;
  ...
  // 공개 API
  return { start, draw, handleClick, handleKey, skip, getSaveData, loadSaveData };

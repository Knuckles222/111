...
const CUTSCENE = (() => {
  ...
  const SCENES = {
    ...
    scene1: [
      ...
      { imgVar: 'girl03', text: "( 이 사람은 나의 대학 동기다. 전에 팀 프로젝트를 한 것으로 친해졌다. 하지만 오늘은 어째선지 굉장히 안절부절못하고 있다.. )", state: "나" },
    ...
    scene3: [
      ...
      { imgVar: 'boy05', text: " 제 친구가 실종된 이유가 이 핸드폰에 있을겁니다. 핸드폰에 있는 바이러스를 없에고 그 이유를 찾게 저를 도와주세요! ", state: "???" },
      { imgVar: 'boy01', text: " 물론, 이 일로 인한 모든 책임은 제가 지겠습니다. ",                                                state: "???" },
      { imgVar: 'boy01', text: " ( 본인이 책임을 지겠다면, 흠... 한 번 해볼까? ) ",                                                state: "나" },
      { imgVar: 'boy01', text: " 그럼 바이러스를 지워 드리겠습니다. 그래도 바이러스가 이미 핸드폰 데이터를 지워버렸을 수도 있으니 너무 기대하진 마세요. ", state: "나" },
    ...
    // ────────────────────────────────────────────────────
    scene4: [
      { imgVar: null, text: "어디서부터 거짓말이지?",                                                                               state: "나" },
      { imgVar: null, text: "다시 한 번 그 사람의 말을 되짚어보고 있다.",                                                           state: "나" },
      { imgVar: null, text: "너무 잘못되었다. 상황이 안 좋다.",                                                                     state: "나" },
      { imgVar: null, text: "일단 생각해보자.",                                                                                     state: "나" },
      { imgVar: null, text: "먼저 그 사람은 나를 당장은 죽이지 못한다.",                                                           state: "나" },
      { imgVar: null, text: "그 장부에 대해 본인은 알아낸 것이 없기 때문이다.",                                                     state: "나" },
      { imgVar: null, text: "그 사람에겐 그게 생명줄이다.",                                                                         state: "나" },
      { imgVar: null, text: "어떻게 할까, 아무 것도 못 본 척 핸드폰을 그냥 돌려줄 수 있다. 아니면 경찰에 신고하거나.",            state: "나" },
      { imgVar: null, text: "전자의 경우 핸드폰을 받고 날 곱게 살려둘 거라는 확신이 안 선다. 이미 많은 사람을 죽여 온 사람이다.", state: "나" },
      { imgVar: null, text: "후자의 경우 이미 날 감시하고 있다면, 경찰이 오기 전에 무슨 화를 당할 지 모른다. 아니면 그 사람만의 다른 대비책이 있겠지.", state: "나" },
      { imgVar: null, text: "생각 할 시간이 별로 없다...",                                                                         state: "나" },
      { imgVar: null, text: "*** 띵 동 ! ***",                                                                                     state: "bell-effect" },
      ...
      { imgVar: null, text: "절 조용히 따라오세요. 가는 길에 저한테서 도망치시더라도 제 조직에게서는 벗어나실 수 없다는 것도 알아두세요.", state: "살인자" },
    ],
  };
  ...
  // ──────────────────────────────────────────────────────
  function start(sceneId, onEnd) {
    currentSceneId = sceneId;
    currentSlide   = 0;
    onEndCallback  = onEnd || null;

    // 이미지 변수명 → 실제 p5.Image 매핑
    // main.js preload() 에서 로드된 전역 변수를 window[name] 으로 참조
    imgMap = {};
    const data = SCENES[sceneId] || [];
    for (const slide of data) {
      if (slide.imgVar && !imgMap[slide.imgVar]) {
        // window[varName] 으로 전역 변수 접근 (p5.js global mode)
        if (typeof window[slide.imgVar] !== 'undefined') {
          imgMap[slide.imgVar] = window[slide.imgVar];
        }
      }
    }
  }
  ...
  function draw() {
    ...
    // ── 배경 이미지 ─────────────────────────────────────
    // 원본 컷신 코드와 동일: backgrounds → room → 캐릭터 → comu → 대사
    ...
    // ── 캐릭터 이미지 (있을 때만) ───────────────────────
    // 원본: image(str.img, 50, 0) — 좌표 그대로 유지
    if (slide.imgVar && imgMap[slide.imgVar]) {
      image(imgMap[slide.imgVar], 50, 0);
    }
    ...
    text(slide.text, VIRTUAL_WIDTH / 2, 840);

    // 화자 이름 (효과음 state 는 표시 안 함)
    if (!EFFECT_STATES.includes(st)) {
      text(st, VIRTUAL_WIDTH / 2, 770);
    }
  ...
  // 공개 API
  return { start, draw, handleClick, handleKey };
